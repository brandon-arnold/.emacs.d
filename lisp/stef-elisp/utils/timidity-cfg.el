...
(defcustom timcfg-cache-file "$SURMULOTDIR/timidity/cache.el"
  "File storing the database of all defined patches"
  :type 'string
  :group 'spfa-paths
  :group 'timidity-cfg)

(defun timcfg-cache-file ()
  (substitute-in-file-name timcfg-cache-file))
...
(defun timcfg-csound-play-soundfont 
  (soundfont bank preset midi-file &optional reverb chorus)
  (require 'csound-x)
  (with-temp-buffer 
    (insert (format 
"<CsoundSynthesizer>
...
"
midi-file (or reverb 0) (or chorus 0) soundfont bank preset))
    (csound-csd-mode)
...
(defun timidity-cfg-define-menu ()
  (easy-menu-define timidity-cfg-mode-menu timidity-cfg-mode-map
    ...
    `("Timidity"
      ...
      ["Refresh cache file" timcfg-refresh-cache-file t]
      "--"
...
(defun timcfg-current-soundfont ()
  (save-excursion
    (beginning-of-line) 
    (when (looking-at "^[0-9 \t]+%font[ \t]+\\(.*\\.sf2\\)") 
	    ...
	    (if (string-match "[0-9]+" command)
	      (append beg
			...
			(list :prog (string-to-number command) 
			      ...
			      :sf2bank (string-to-number (fourth items))
			      :sf2preset (string-to-number (fifth items)))
			(list :prog (string-to-number command) 
...
(defvar timidity-patches 
  (if (file-exists-p (timcfg-cache-file))
      (car (read-from-string 
	    (with-temp-buffer-to-string
	      (insert-file-contents (timcfg-cache-file)))))
    (timcfg-parse-file (surmulot-timidity-cfg))))
   
(defun timcfg-refresh-cache-file ()
  (interactive)
  (with-temp-file (timcfg-cache-file)
    (insert (prin1-to-string 
	     (setq timidity-patches
		   (timcfg-parse-file (surmulot-timidity-cfg)))))))

;(find-file (timcfg-cache-file))

(defun timcfg-find-patches (&rest keyvals)
  (remove-if-not
   (lambda (spec) 
     (loop for key in keyvals by 'cddr
	   always (eq (plist-get spec key) (plist-get keyvals key))))
   timidity-patches))

;TEST (timcfg-find-patches :bank 60 :prog 10) => '((:bank 60 :prog 10 :sf2 "d:/devel/surmulot/soundfonts/bottleblow.sf2" :sf2bank 0 :sf2preset 0 :name "Bottle Blow"))

(defun timcfg-find-patch (&rest keyvals)
  (find-if
   (lambda (spec) 
     (loop for key in keyvals by 'cddr
	   always (eq (plist-get spec key) (plist-get keyvals key))))
...
;TEST (timcfg-find-patch :bank 60 :prog 10) => '(:bank 60 :prog 10 :sf2 "d:/devel/surmulot/soundfonts/bottleblow.sf2" :sf2bank 0 :sf2preset 0 :name "Bottle Blow")

(defun timcfg-patch-name (bank program)
  (plist-get (timcfg-find-patch :bank bank :prog program) :name))

;TEST (timcfg-patch-name 60 10) => "Bottle Blow"

(defun drumset-sf2 (drumset)
  (plist-get (timcfg-find-patch :drumset drumset) :sf2))

;TEST (drumset-sf2 78) => "d:/devel/surmulot/soundfonts/ns_kit7free.sf2"
