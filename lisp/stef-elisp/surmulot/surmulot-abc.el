...
;;; surmulot-abc.el --- Major mode for editing abc music files
;;; This a modified version of abc-mode.el for Surmulot by SPFA

;; Original abc-mode.el is copyright (C) 2002  Matthew K. Junker
...
(defcustom abc-executable 
  (if (eq system-type 'windows-nt)
      (expand-file-name "bin/abctools/abc2ps" (surmulot-directory))
    "abc2ps")
  "*Location of the abc2ps executable."
  :type 'string 
...
(defcustom abc-midi-executable 
  (if (eq system-type 'windows-nt)
      (expand-file-name "bin/abctools/abc2midi" (surmulot-directory))
    "abc2midi")
  "Location of the abc2midi executable."
  :type 'string 
  :group 'surmulot-paths
  :group 'abc-abc2midi)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defcustom abc-abc2abc-executable
  (if (eq system-type 'windows-nt)
      (expand-file-name "bin/abctools/abc2abc" (surmulot-directory))
    "abc2abc")
  "*Location of the abc2abc executable."
  :type 'string 
  :group 'surmulot-paths
  :group 'abc-abc2ps)
...
selected option set."
  ...
  (let ((name (abc-preprocess-buffer argp)))
    (shell-command
      (concat (shell-quote-argument abc-executable)
    ...
    ;; ===== on enchaine sur l'affichage du ps 
    (shell-command "c:/gstools/gsview/gsview32 tmp.ps &")
...
(defun abc-compare (argp)
  (interactive "P")
  (abc-run-ABCParser argp)
  (abc-run-abc2midi argp))

(defun abc-compare-one (argp)
  (interactive "P")
  (abc-run-ABCParser-one argp)
  (abc-run-abc2midi-one argp))

(defun abc-run-ABCParser (argp)
  "Run ABCParser>>#abcToMidi: on the current buffer.
...
Optional argument OPTIONS is program flags."
  (interactive "P")
  (save-buffer)
  (surmulot-handle-midi-files
   (changed-files
    (let ((name (abc-preprocess argp buffer-file-name t)))
      ($$ `[ABCParser abcToMidi: ,name])))))

(defun abc-run-ABCParser-one (argp)
  "Run ABCParser>>#abcToMidi:tune: on the current buffer.
...
Optional argument OPTIONS is program flags."
  ...
  (surmulot-handle-midi-files
   (changed-files
    (let ((name (abc-preprocess argp buffer-file-name t)))
      (shell-command
	(concat (shell-quote-argument abc-midi-executable) " "
		(file-name-nondirectory name)
		(if options (concat " " options) "")))))))

(defun abc-run-file (abc-file)
  (let ((surmulot-midi-file-handler 'identity))
    (save-excursion
      (set-buffer (find-file-noselect abc-file))
      (abc-mode)
      (abc-run-abc2midi nil))))

(defun abc-run-text (text)
  (let ((abc-file (cscsd-make-temp-file "tmp" nil ".abc")))
    (with-temp-file abc-file
      (insert text))
    (abc-run-file abc-file)))
...
(easy-menu-define
  ...
  (list "abc"
	...
	(list"Postscript"
        ...
        ["Run ABCParser (buffer)"        abc-run-ABCParser       t]
	["Run ABCParser (song)"        abc-run-ABCParser-one       t]
	["Compare abc2midi and ABCParser (buffer)"        abc-compare t]
	["Compare abc2midi and ABCParser (song)"        abc-compare-one t]
        "--"
...
(provide 'surmulot-abc)
;;; surmulot-abc.el ends here
