...
(defvar surman-separator (make-string 1 11)
  "One-character string used to separate items in a TCP request")

(defvar surman-end-separator (make-string 2 11)
...
(defvar squeak-namespace '(top)
  "A list of symbols defining the lexical namespaces hierarchy used
by evaluation of Smalltalk code within Emacs. See macro `let$' and function
`surman-ask-squeak'.")
...
;;     (when
;; 	(setq surman-process
;; 	      (make-network-process :name "surman" :buffer nil
;; 				    :host "127.0.0.1" :service surman-port
;; 				    :sentinel 'surman-process-sentinel
;; 				    :filter 'surman-process-filter))
;;       (surman-cancel-daemon))))
...
(defun surman-ask (manager question &optional namespace)
  ""
  (let (my-counter)
	...
	(process-send-string surman-process 
			     (concat question surman-separator
				     manager surman-separator
				     "Q" surman-separator
				     (format "%s" my-counter)
				     (if namespace
...
(defun surman-ask-squeak (question)
  (interactive "sSqueak> ")
  (let ((ans (squeak-decode
	      (surman-ask "squeak_manager" 
			 (squeak-encode question t)
			 squeak-namespace) ;; specific to Squeak
	      t)))
...
(defun surman-ask-clojure (question)
  (interactive "sClojure> ")
  (let ((ans (surman-ask "clojure_manager" question)))
    (if (interactive-p) (message ans) ans)))
...
(defun clojure-repl ()
  (interactive)
  (surman-repl 'surman-ask-clojure))
