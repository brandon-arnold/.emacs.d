      ...
      (let ((image (concat (expand-file-name 
			    (substitute-in-file-name 
			     (file-name-sans-extension 
			      (first squeak-tcpip-default-images))))
			   ".image")))
	 ...
	 (scommand
	  (format 
	   ...
	   (base64-encode-string
	    (format 
	     "\"free\" MuODeployer makeWidget: (%s) extent: (%s@%s) port: %s" 
	     command
...
(defun muo-docbooks ()
  (setq muo-docbooks
	(delete-if
	 ...
	 (read ($: Surmulot docBooks)))))

(defun muo-doc (&optional reference page-number)
  "Open the muo documentation REFERENCE in a dedicated Squeak window.
For a list of available references, see variable `muo-docbooks'.
REFERENCE may be a regexp, in which case the first matching item in
variable `muo-docbooks' will be selected.
When PAGE-NUMBER is not nil, open that page of the documentation."
  (interactive 
   (list (completing-read "Open documentation: " (or muo-docbooks
						     (muo-docbooks)))))
  (let ((ref (if (member reference (or muo-docbooks
				       (muo-docbooks)))
	       reference
	     (find-if (lambda (d) (string-match reference d))
		      muo-docbooks))))
    (if (null ref)
	(message "No documentation found matching: %s" reference)
      (if page-number
	  (surmulot-make-widget
	   (format "MuO openDocumentation: #'%s' onPage: %s" 
		   ref page-number))
	  (surmulot-make-widget
	   (format "MuO openDocumentation: #'%s'" ref)))
      ref)))
...
(defun surmulot-widgets-add (name command)
  (let ((widget (surmulot-make-widget "Morph new color: Color transparent")))
	...
	(surmulot-widget-do widget (format "Surmulot widget: (%s)" command))
      (surmulot-widget-do widget `(Surmulot widget: ,command)))
    (surmulot-widget-delete-output widget)
    widget))
...
(defun surmulot-widget-do (widget command)
  (squeak-eval (if (stringp command)
		   command
		 (print-smalltalk-to-string command))
	       (surmulot-widget-port widget)))

(put 'surmulot-widget-do 'lisp-indent-function 1)
...
(defun surmulot-widget-clone (widget)
  (let ((name (surmulot-widget-name widget))
	(str (surmulot-widget-do widget
	      '(Widget biggerSurmulotWidget asEncodedString asOneLine))))
    (surmulot-widgets-add name `(,str reconstituteObject))
    (surmulot-widget-delete-output widget)))
...
(defun surmulot-widget-send-to-muo (widget)
  (let ((str (surmulot-widget-do widget
	       `(Widget biggerSurmulotWidget asEncodedString asOneLine))))
    (surmulot-widget-delete-output widget)
    ($$ `[,str reconstituteObject quitBehavingAsSurmulotWidget openInWorld])))
