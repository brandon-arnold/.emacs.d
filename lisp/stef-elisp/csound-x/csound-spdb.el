...
DEPTH is the current indentation depth."
  ...
  (cond
   ...
   ((equal tag-type 'orc)
    ...
    (when (csspdb-csd-contains-p "^[ \t]*py.*{{")
      (csspdb-subnode "Python code" 'python))
    (when (csspdb-csd-contains-p "^[ \t]*lua.*{{")
      (csspdb-subnode "Lua code" 'lua)))
   ;;; level 2
   ...
   ((equal tag-type 'opcodes)
    (dolist (opcode (csspdb-within-csd-buffer
      ...
      (speedbar-make-tag-line
       ...
       'default depth)))
   ;;
   ((equal tag-type 'python)
    (dolist (py-section (csspdb-within-csd-buffer
		     (let (py-sections)
		       (save-excursion
			 (cscsd-goto-orc)
			 (while (re-search-forward "^[ \t]*py.*{{" 
						   (cscsd-orc-end) t)
			   (add-to-list 'py-sections
                                        (list (point) 
					      (re-search-forward
					       "}}" 
					       (cscsd-orc-end) t))
		      ...
		      py-sections)))
      (speedbar-make-tag-line
       'bracket ?> 'csspdb-goto-char (car py-section)
       (format "L%s {{-edit-}}"  
	       (csspdb-within-csd-buffer (line-number-at-pos (car py-section))))
       'csspdb-edit-region (list (car py-section) 
				 (- (cadr py-section) 2)
				 'python-mode)
       'default depth)))
   ;;
...
(defun csspdb-goto-char (text pos indent)
  "When user clicks on TEXT, go to position POS.
The INDENT level is ignored."
  (csspdb-within-csd-buffer
   (goto-char pos)))
...
(defun csspdb-edit-region (text region indent)
  "When user clicks on TEXT, edit REGION as Python code
in an indirect buffer.
REGION is a list of three items: the region start and end, the major
mode of the indirect buffer.
The INDENT level is ignored."
  (csspdb-within-csd-buffer
   (switch-to-buffer ;-other-frame
    (make-indirect-buffer
     (current-buffer)
     (format "Code at L%s in %s"
	     (line-number-at-pos (car region)) (buffer-name))))
   (funcall (caddr region))
   (setq header-line-format nil)
   (narrow-to-region (car region)
		     (cadr region))))
