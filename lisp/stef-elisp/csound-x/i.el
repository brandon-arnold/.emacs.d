  ...
  `(let ((with-pfields-stream:::parent-pfstream-end-time pfstream-end-time)
     ...
     (let* ((pfstream-custom-pf-keys (copy-sequence pfstream-custom-pf-keys))
       ...
       (flet ,(mapcar 
	       (lambda (ea)
		 `(,(car ea) (&rest args) 
		   (apply (symbol-function ',(cadr ea)) args)))
	       pfstream-letf-spec)
...
;TEST (with-temp-buffer-to-string (i "named" 0 5)) => "i \"named\"\t0\t5\n"
...
(defun i-primitive (&rest p-fields)
  (insert "i "
	  (mapconcat (lambda (pf) 
		       (if (keywordp pf) (substring (format "%S" pf) 1)
			 (if (stringp pf) (format "%S" pf)
			  ...
			  (if (assoc realkey pfstream-custom-pf-keys)
			    ...
			    (list realkey
				  (if (functionp (eval realk-val))
				    ...
				    realk-val))))))
	(setq expanded-pf 
	      (append expanded-pf (list key (plist-get pf key))))))))

(defun add-custom-key (new-key-def) 
  "Register a new custom key for setting pfields in the stream
NEW-KEY-DEF is a list \(KEY VAL) where VAL is a plist or a function of one
variable returning a plist: KEY is a keyword which will be expanded into that
plist.
when VAL is a function its single argument will be the value associated to KEY
when it is invoked; this argument is not restricted to any type"
  ...
  (when (assoc key pfstream-custom-pf-keys)
    (setq pfstream-custom-pf-keys
	  (assq-delete-all key pfstream-custom-pf-keys)))
...
;TEST (cseel-expand-generic-statement '(ao5d50)) => '(ao5 :dur 50)
