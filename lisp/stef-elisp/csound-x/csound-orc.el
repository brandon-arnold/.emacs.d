...
(defcustom csound-orc-indent-fix-functional-syntax nil
  "If not nil, then indenting will remove any space between a function
and its arguments, for compatibility with the Csound 6 parser." 
  :type 'boolean
  :group 'csound-orc)
...
(defcustom csound-tab-stops 
  '(12 15 22 24 26 28 30 32 34 36 38 40 42 44 46 48)
  "Positions for columns layout."
  :type '(repeat integer)
; :set ...... do something here (restart csound-csd-mode everywhere ?)
  :group 'csound-orc)
...
(defcustom csound-orc-box-foreign-code nil
  "When non-nil, embedded python and lua code is hidden from view
and replaced by a button allowing edition in an indirect buffer."
  :type 'boolean
  :group 'csound-orc)
	  ...
	  (cond
	   ...
	   ;;--------------
		...
		(csound-shrink-horizontal-space))
	      (tab-to-tab-stop))
	    ;; fix functional syntax (for csound 6 parser)
	    (when csound-orc-indent-fix-functional-syntax
	      (csound-fix-functional-syntax))
	    ;; tabulate left comment if any
	    (csound-tabulate-left-comment))))))))

(defun csound-fix-functional-syntax ()
  "Before Csound 6, space was allowed between a function and its arguments.
Not any more, so fix it where it happens."
  (while (re-search-forward "[a-zA-Z0-9]\\( +\\)(" 
			    (or (save-excursion (search-forward ";"))
				(point-at-eol))
			    t)
...
(defun csound-font-lock-keywords ()
  (list  
   ...
   `(,csound-regexp-python-opcodes . 'csound-python-opcode-face)
   `(,csound-regexp-lua-opcodes . 'csound-lua-opcode-face)
   ...
   '("{{[[:ascii:]]*?}}"  0 'csound-string-face t)
   ;; comments 
   '("\\(;[^|\n\r].*$\\)" 1 'csound-comment-face t)
   '("\\(/\\*[[:ascii:]]*?\\*/\\)" 1 'csound-long-comment-face t)
   '("\\(/\\*|[[:ascii:]]*?|\\*/\\)" 1 'csound-meta-comment-face t)
   ;; python code 
   '("py.*{{\\([[:ascii:]]*?\\)}}" 1 'csound-python-code-face t)
   ;; lua code 
   '("lua.*{{\\([[:ascii:]]*?\\)}}" 1 'csound-lua-code-face t)
   ;; meta-labels
...
(dolist 
    (fspec
     '( ;; face      "colour"	"background"   bold underline 
       ...
       (csound-meta-comment-face "darkGreen" "grey97"  t nil "meta comments")
       (csound-long-comment-face "darkGreen" nil nil nil "comments")
       (csound-python-opcode-face "orangered2" "white" t nil "Python opcode")
       (csound-lua-opcode-face "orangered2" "white" t nil "Lua opcode")
       (csound-python-code-face "DeepSkyBlue4" "beige" nil nil "Python code")
       (csound-lua-code-face "DeepSkyBlue4" "beige" nil nil "Lua code")
...
(defun csound-install-orc-menu ()
  (easy-menu-define csound-orc-menu csound-orc-mode-map
    ...
    `("ORC"
      ...
      ["Edit instrument at point by itself"            cscsd-instr-alone  (cscsd-this-instrument)]
...
;;        (and (cscsd-non-indirect-buffer-p) (not (cscsd-buffer-is-a-csd-p)))]
      "--"
      ["Play orchestra via temp CSD" 
       (let ((cscsd-current-processing "csound dac"))
	 (cscsd-process-scorc-via-temp-csd)) t]
...
(defun csound-get-kr ()
  (or (csound-orc-global-value "kr")
      (/ (csound-orc-global-value "sr")
	 (csound-orc-global-value "ksmps"))))
...
;;======================================================================
;                       boxing python & lua code
;;======================================================================

(require 'button)

(defun csound-orc-button-mask (beg end label action)
  "Private.
Note that the character after END will become inaccessible.
This works best when character after END is a whitespace.
See function `csound-orc-box-at-point' for example"
    (let ((map (make-sparse-keymap))
	  (tov (make-overlay beg (1+ end))))
      (define-key map [mouse-2] 'csound-orc-button-unmask)
      (define-key map [mouse-3] 'csound-orc-button-unmask)
      (define-key map [mouse-1] action)
      (overlay-put tov 'mask t)
      (overlay-put tov 'display label)
      (overlay-put tov 'local-map map)
      (overlay-put tov 'field 'button)
      (overlay-put tov 'help-echo 
		   (lambda (w o p) (field-string-no-properties p)))
      (overlay-put tov 'priority 100)
      (overlay-put tov 'face 'custom-button)
      (overlay-put tov 'mouse-face 'custom-button-mouse)))

(defun csound-orc-button-unmask (event)
  (interactive "e")
  (save-excursion
    (mouse-set-point event)
    (let ((ov (find-if (lambda (ov)
			 (overlay-get ov 'mask))
		       (overlays-at (point)))))
      (when ov
	;; (add-text-properties (field-beginning) (field-end)
	;; 		     '(font-lock-face csound-lua-code-face))
	(delete-overlay ov)))))

(defun csound-orc-button-edit (mode label event)
  (save-excursion
    (mouse-set-point event)
    (let* ((ov (find-if (lambda (ov)
			 (overlay-get ov 'mask))
		       (overlays-at (point))))
	   (buff (or (buffer-base-buffer (current-buffer))
		     (current-buffer)))
	   (beg (field-beginning))
	   (obuff (overlay-get ov 'edit-buffer))
	   (ibuff (or (and (buffer-name obuff) obuff)
		      (make-indirect-buffer 
		       buff 
		       (format "%s at L%s in %s" 
			       label (line-number-at-pos beg) buff))))
	   (end (- (field-end) 1)))
      (switch-to-buffer ibuff)
      (narrow-to-region beg end)
      (funcall mode)
      (overlay-put ov 'edit-buffer ibuff))))

(defun csound-orc-button-python-edit (event)
  (interactive "e")
  (csound-orc-button-edit 'python-mode "python" event))

(defun csound-orc-button-lua-edit (event)
  (interactive "e")
  (csound-orc-button-edit 'lua-mode "lua code" event))

(defun csound-orc-box-at-point ()
  (save-excursion
    (beginning-of-line)
    (when (looking-at "^[ \t]*\\(py\\|lua\\).*{{")
      (let ((code (match-string-no-properties 1))
	    (beg (search-forward "{{"))
	    (end (cond
		    ((re-search-forward
		      "^}}"
		      (cscsd-orc-end) t) (- (point) 3))
		    ((re-search-forward
		      "[\t ]}}"
		      (cscsd-orc-end) t) (- (point) 3))
		    ((re-search-forward
		      "}}"
		      (cscsd-orc-end)) (- (point) 2)))))
	(csound-orc-button-mask beg end
		     (concat " edit " 
			     (if (string= code "py") "python" "lua")
			     " code"
			     (if (= end (- (point) 2)) " }" " "))
		     (if (string= code "py")
			 'csound-orc-button-python-edit
		       'csound-orc-button-lua-edit))
	t))))

(defun csound-orc-box-at-point-with-control ()
  (interactive)
  (save-excursion
    (beginning-of-line)
    (when (csound-orc-box-at-point)  
      (beginning-of-line)
      (search-forward "{")
      (make-button (- (point) 1) (point)
      		   'face 'custom-button
      		   'mouse-face 'custom-button-mouse
      		   'help-echo (lambda (w o p)
      				(if (field-at-pos (+ 2 p))
      				    "Unbox code section"
      				  "Box code section"))
      		   'follow-link t
      		   'action (lambda (b)
      			     (if (field-at-pos (+ 2 (button-start b)))
      				 (save-excursion
      				   (goto-char (+ 2 (button-start b)))
      				   (csound-orc-button-unmask nil))
      			       (csound-orc-box-at-point)))))))

(defun csound-orc-unbox-at-point ()
  (interactive)
  (save-excursion
    (beginning-of-line)
    (when (looking-at "^[ \t]*\\(py\\|lua\\).*{{")
...
(defun csound-orc-box-code-sections ()    
  (interactive)
  (save-excursion
    (cscsd-goto-orc)
    (while (search-forward "{{" (cscsd-orc-end) t)
      (csound-orc-box-at-point-with-control))))

;; (defun csound-orc-unbox-code-sections ()
;;   (interactive)
;;   (save-excursion
;;     (cscsd-goto-orc)
;;     (while (search-forward "{{" (cscsd-orc-end) t)
;;       (forward-char -1)
;;       (csound-orc-unbox-at-point))))
