  ...
  (dolist (var cscsd-hidden-environment-variables)
    (let ((val (symbol-value (intern (concat "cscsd-hidden-" var))))
	  ...
	  (setq val (if (and val current)
			(concat val path-separator current)
		      current)))
      (if val
		...
		(defun ,varsymb ()
		  (when ,varsymb
		    (substitute-in-file-name
		     (car (split-string ,varsymb path-separator)))))
		(defun ,varsymbfull ()
		  (when ,varsymb
		    (reverse (mapcar 'substitute-in-file-name
				     (split-string ,varsymb path-separator)))))
...
(defun cscsd-actual-csoundrc (&optional v6)
...
If V6 is non-nil, consider CSOUND6RC and .csound6rc respectively."
  (let* ((csoundrc (if v6 ".csound6rc" ".csoundrc"))
	 (loc (expand-file-name csoundrc 
			       (buffer-file-name (current-buffer)))))
    (or (and (file-exists-p loc) loc)
	(if v6 (cscsd-CSOUND6RC) (cscsd-CSOUNDRC))
	(expand-file-name (concat "~/" csoundrc)))))
...
(defcustom cscsd-odac-settings ""
  "Csound command line options to be added to real-time processing."
  :type 'string
  :group 'csound-csd)
...
(defcustom cscsd-process-file
  '(("csound <CsOptions> csd" 
    ...
    ("csound wav (sync.)"
     "$csound -dWo %s %s")
    ("csound dac"
     "$csound -dodac $dacoptions %s &")
    ("csound midi to dac"
     "$csound -dodac $dacoptions %s -TF %s &")
    ("csound wav + play"     
     (lambda (csd-file output-file)
       (set-cscsd-post-csound-log-action 
	(lambda ()
	  (cscsd-play-audio (cscsd-audio-file-from-log))))
       (cscsd-call-csound "$csound -dWo %s %s &" csd-file output-file)))
    ("csound wav + edit"     
     (lambda (csd-file output-file)
       (set-cscsd-post-csound-log-action 
	(lambda ()
	  (cscsd-edit-audio (cscsd-audio-file-from-log))))
       (cscsd-call-csound "$csound -dWo %s %s &" csd-file output-file)))
    ("publish csd" cscsd-publish-csd-file))
...
\"process region\" items from the \"CSD\" menu.
...
(put 'cscsd-current-processing 'permanent-local t)
...
(defcustom csound-temporary-file-directory
  temporary-file-directory
  "The directory for writing temporary files for Csound-x."
  :type 'file
  :group 'csound-csd
  :group 'csound-x-applications-paths)

(defun csound-temporary-file-directory ()
  (substitute-in-file-name csound-temporary-file-directory))

(defun cscsd-make-temp-file (prefix &rest rest)
  (apply 'make-temp-file
	 (expand-file-name prefix (csound-temporary-file-directory))
	 rest))
...
(defcustom csound-x-disable-smart-semicolon t
...
(defcustom csound-x-disable-smart-colon t
...
;  (add-to-invisibility-spec 'org-link)
  ...
  ;;
  (when csound-orc-box-foreign-code
    (csound-orc-box-code-sections))
  ;;
  ...
  (let ((csdp (cscsd-buffer-is-a-csd-p)))
    ...
    `("CSD"
      ,@(when (and csdp (featurep 'surmulot))
          (surmulot-csd-menu)) ;; from surmulot.el
	  ...
	  '(("Structure"
	     ...
	     "--"
	      ...
	      (cscsd-non-indirect-buffer-p)]
	     "--"
	     ["Box python/lua code sections"  csound-orc-box-code-sections t]
	     ["Unbox python/lua code sections"  csound-orc-unbox-code-sections t]
             "--"
	     ["Build a true csd" cscsd-auto-import
	      (cscsd-non-indirect-buffer-p)]
	     ["Build a csd from clipboard" cscsd-import-from-clipboard t])))
      ["Display all tables" csft-display-buffer-tables t]
...
;      "--"
      ...
      ["Edit CSOUND6RC file" cscsd-edit-csound6rc t]
      ...
      [,(concat " ... " (if (cscsd-audio-available-p) 
			   (file-name-nondirectory cscsd-latest-audio-file)
			    ""))
       cscsd-edit-audio cscsd-latest-audio-file]
...
(defvar cscsd-latest-csd-file nil
  "Record the name of the latest compiled CSD file.") 

(defvar cscsd-latest-csd-contents nil
  "Record the contents of the latest compiled CSD file.") 
...
(put 'cscsd-latest-audio-file 'permanent-local t)
...
(put 'cscsd-latest-midi-file 'permanent-local t)
  ...
  (let ((rcommand
	 (replace-regexp-in-string
	  ...
	  (replace-regexp-in-string 
	   ...
	   (replace-regexp-in-string 
	    "$DACOPTIONS" cscsd-odac-settings
	    command)))))
...
(defcustom cscsd-protected-csound-process nil
  "When nil, the current Csound process is automatically interrupted upon
invocation of another Csound command."
  :type 'boolean
  :group 'csound-csd)  

(defcustom cscsd-csound-log-pops-up t
  "If non-nil, the Csound log buffer appears in a specific pop-up frame."
  :type 'boolean
  :group 'csound-csd)  
...
AUDIO-FILE and to the appropriate MIDI file, if any."
  (let* ((syscall (cscsd-csound-command syscall omacros smacros))
    ...
    (message "Interpreted Csound command:")
    (message 
     (if (= (count ?% syscall) 3)
	 (format syscall
...
(defun cscsd-examine-last-csd ()
  "Display in a specific buffer the contents of the last CSD file to have been
compiled via function `cscsd-call-csound"
  (interactive)
  (when cscsd-latest-csd-contents
    (switch-to-buffer
     (get-buffer-create 
      (generate-new-buffer-name (concat "Contents of " cscsd-latest-csd-file))))
    (insert "; contents of (possibly vanished) " 
	    cscsd-latest-csd-file "\n\n")
    (insert cscsd-latest-csd-contents)
    (goto-char (point-min))
    (csound-csd-mode)))
...
The command output appears in a buffer named `cscsd-csound-log-buffer-name'."
  (setq cscsd-latest-csd-file csd-file)
  (setq cscsd-latest-csd-contents
	(with-temp-buffer-to-string (insert-file-contents csd-file)))
  (save-window-excursion
    ...
    (shell-command cscsd-last-csound-invocation
		   cscsd-csound-log-buffer-name)))

(defadvice shell-command (around hack-exit 
				 (command &optional output-buffer error-buffer))
  "Allows automatic interruption of the current Csound process
whenever another Csound command is called.
This can be disabled by setting variable `cscsd-protected-csound-process'
to a non-nil value"
  (let (pproc)
    (when (and (null cscsd-protected-csound-process)
	       (string= (if (stringp output-buffer)
			    output-buffer
			  (buffer-name output-buffer))
			cscsd-csound-log-buffer-name)
	       (setq pproc (get-buffer-process cscsd-csound-log-buffer-name)))
      (kill-process pproc)
      (loop while (get-buffer-process cscsd-csound-log-buffer-name)
	    do (sit-for .05)))
    ad-do-it))
...
associated to `cscsd-current-processing' in `cscsd-process-file'. It can 
alternatively be set explicitely via argument PROCESSING.
...
(defmacro with-cscsd-copy-for-processing (temporary-files &rest body)
  "Evaluate BODY within a copy of the current buffer, save that copy
in the first file in TEMPORARY-FILES, register all TEMPORARY-FILES for
deletion after the next Csound invocation, return the modified buffer
file name"
  (declare (indent 1))
  `(let ,(append temporary-files 
	...
	`(lambda ()
	   (mapcar (lambda (f)
		   ...
		   ',(list ,@(mapcar 'car temporary-files)))))
       (widen)
       (with-temp-file ,(caar temporary-files)
	 (insert-buffer-substring source-buffer)
	 ,@body)
       ,(caar temporary-files))))
...
(defun cscsd-buffer-CSD-file (&optional save-if-modified)
  "Return the buffer file name if that file name has extension \".csd\"
and if the buffer is not modified \(or if SAVE-IF-MODIFIED is non-nil in
which case the buffer will be saved), else return nil."
 (let ((filename (buffer-file-name (buffer-base-buffer))))
   (when (and filename
	      (string-match "\\.csd$" filename)
	      (or (not (buffer-modified-p))
		  (and save-if-modified
		       (progn (save-buffer) t))))
     filename)))
...
(defmacro with-cscsd-processing (audiofile midifile processing &rest body)
  ""
  (declare (indent 2))
  (let ((rootfile (make-symbol "rootfile"))
	(csdfile (make-symbol "csdfile"))
	(val (make-symbol "val")))
    `(let* ((,rootfile (cscsd-make-temp-file "cscsd-"))
...
(defun cscsd-safe-file-name (file) ;; util. (should be elsewhere)
;; to do: replace spaces with %20 ?
  (if (eq system-type 'windows-nt)
      (or (w32-short-file-name (expand-file-name file))
	...
	(format (substitute-in-file-name cscsd-shell-edit-audio)
		(cscsd-safe-file-name
		 (expand-file-name (or audio-file
				       cscsd-latest-audio-file) 
				   (cscsd-actual-sfdir))))
	 (generate-new-buffer-name " *audio editor*")))
    (message "Don't know what file to edit !")))

(defmacro really-save-window-excursion (&rest body)
  "Make `save-window-excursion' actually work from a window displayed
in an unsplittable frame"
  `(let ((frame-unsplittable-parameter
	  (frame-parameter (window-frame (selected-window)) 'unsplittable)))
     (set-frame-parameter (window-frame (selected-window)) 'unsplittable nil)
     (save-window-excursion
       ,@body)
     (set-frame-parameter (window-frame (selected-window)) 
			  'unsplittable frame-unsplittable-parameter)))
...
the same root name as the current buffer file name, or else if we can get
something from the last csound log via `cscsd-audio-file-from-log'.
Returns either nil or the full audio file name.
	   ...
	   (setq cscsd-latest-audio-file 
		 (car (directory-files (cscsd-actual-sfdir) nil
				 ...
				 (file-name-nondirectory 
				  (buffer-file-name))))))))
      (and (stringp (cscsd-audio-file-from-log))
	   (file-exists-p 
	    (expand-file-name (cscsd-audio-file-from-log) (cscsd-actual-sfdir)))
	   (setq cscsd-latest-audio-file (cscsd-audio-file-from-log)))))
...
(defun cscsd-scan-log-buffer ()
  ...
  (save-excursion
    ...
    (let* ((csd 
	    (save-excursion 
	      (when (re-search-forward "UnifiedCSD:[ ]*\\(.*\\)$" nil t)
		(cscsd-add-text-action
		 (match-beginning 1) (match-end 1)
		 `(find-file ,(match-string-no-properties 1)))
		(match-string-no-properties 1))))
	   (orc 
	    (save-excursion 
	      (when (re-search-forward "orchname:[ ]*\\(.*\\)$" nil t)
		(match-string-no-properties 1))))
           (filename (or (and csd (file-exists-p csd) (expand-file-name csd))
                         (and orc (file-exists-p orc) (expand-file-name orc))))
           source)
      (when (and 
	     (save-excursion (re-search-forward "^error:" nil t))
	     (setq source 
		   (and filename
			(or (loop for buffer in (buffer-list)
	...
	(while (re-search-forward "^error:.*\\(line \\(.*\\)\\):" nil t)
	  ...
	  (cscsd-add-text-action
	   ...
	   `(progn
	      (switch-to-buffer-other-window ,source)
	      (goto-char (point-max))
	      (search-backward ,(substring-no-properties
				 (thing-at-point 'line) 0 -1))))))
      (save-excursion
	(goto-char (point-max))
	(when (re-search-backward "written to \\(.*\\) (" nil t)
	  (cscsd-add-text-action
	   (match-beginning 1) (match-end 1)
	   `(cscsd-edit-audio ,(match-string 1))))))))	 

(defun cscsd-add-text-action (beg end action)
...
(defun cscsd-audio-file-from-log ()
  (save-excursion
    (when (get-buffer cscsd-csound-log-buffer-name)
      (set-buffer cscsd-csound-log-buffer-name)
      (goto-char (point-max))
      (when (re-search-backward "written to \\(.*\\) (" nil t)
	(match-string-no-properties 1)))))
...
(defun cscsd-instruments (&optional up-to no-opcodes)
  (let (iando
	(regexp (if no-opcodes
		    "^[ \t]*\\(instr\\)[ \t]+\\([_0-9a-zA-Z,$]*\\)"
		  "^[ \t]*\\(instr\\|opcode\\)[ \t]+\\([_0-9a-zA-Z,$]*\\)")))
    (save-excursion
      (goto-char (point-min))
      (while (re-search-forward regexp up-to t)
...
(defun cscsd-edit-csoundrc (&optional v6)
  (interactive)
  (find-file-other-window (cscsd-actual-csoundrc v6))
  (unless (file-exists-p (cscsd-actual-csoundrc v6))
...
(defun cscsd-edit-csound6rc ()
  (interactive)
  (cscsd-edit-csoundrc t))

(defcustom cscsd-hard-code-buffer-local-processing 'ask
  "Should emacs hard-code the value of variable `cscsd-current-processing'
when buffer-local ? Either always, never or to be asked interactively."
...
(defun cscsd-make-processing-local (str)
  ...
  (when (or (and (eq cscsd-hard-code-buffer-local-processing 'ask)
		 (y-or-n-p "Hard-code buffer-local value in file ?"))
	    (eq cscsd-hard-code-buffer-local-processing t))
    (save-excursion
      (save-restriction
	(add-file-local-variable-prop-line
	 'cscsd-current-processing cscsd-current-processing))))
  (cscsd-build-menus))
...
(defvar-local csound-score-binary nil
  "Which executable is responsible for rendering a Csound score from buffer")

(put 'csound-score-binary 'permanent-local t)
...
(defun cscsd-split-buffer (&optional orc-name &optional sco-name)
  ...
  (let ((score-binary (cscsd-score-interpreter)))
	...
	(save-excursion
	  (copy-to-buffer (get-buffer-create sco-name)
			  ...
			  (cscsd-sco-end))
	  (set-buffer sco-name)
	  (if (null score-binary)
	      (csound-sco-mode)
	    (setq csound-score-binary score-binary)
	    (normal-mode))
...
(defun cscsd-wrap-buffer (&optional use-new-buffer)
...
do nothing if we already are in a csd buffer"
  ...
  (unless (cscsd-buffer-is-a-csd-p)
    (let* ((comp-ext (if (cscsd-buffer-is-a-sco-p) ".orc" ".sco"))
	   ...
	   (root-file-name (when (buffer-file-name)
			     (file-name-sans-extension (buffer-file-name))))
	   (csd-buffer-name (if use-new-buffer
				(make-temp-name root-name)
			      root-name))
...
(defun cscsd-goto-sco ()
  ...
  (interactive)
  (goto-char (point-min))
  (re-search-forward "<CsScore[^>]*>" nil t)
  (point))
...
(defun cscsd-go-after-orchestra ()
  "Move point just after </CsInstruments>, return point"
  (interactive)
  (goto-char (point-min))
  (search-forward "</CsInstruments>")
  (point))
...
(defun buffer-csd-string (&optional buffer)
  (save-excursion
...
(defun cscsd-goto-end-of-cssynth ()
  "Move point just before </CsoundSynthesizer>, return point"
  (interactive)
  (goto-char (point-max))
  (search-backward "</CsoundSynthesizer>" nil t)
  (point))
...
(defun cscsd-overwrite-score (score &optional buffer)
  "Replace the current score with SCORE"
  (save-excursion
...
(defun cscsd-append-score (score &optional buffer)
   "Append SCORE to the current score"
  (save-excursion
    (when buffer (set-buffer buffer))
...
(defun cscsd-make-template (&optional name void score-binary)  
  (interactive)
  (if (and (get-buffer (setq name (or name "temp.csd")))
    ...
    (unless (and (get-buffer name)
      ...
      (cscsd-insert-template void score-binary)
      t)))
...
(defun cscsd-buffer-from-squeak-string (str)
  "Edit STR, a base64 encoded string in Squeak coding system, as a CSD
in a newly created buffer; return that buffer name"
  (save-excursion
    (set-buffer (get-buffer-create (cscsd-make-temp-name)))
    (insert (squeak-decode str))
    (goto-char (point-min))
    (csound-csd-mode)
    (buffer-name)))
...
`cscsd-orc-delimiter-regexps' and `cscsd-sco-delimiter-regexps'"
  ...
  (unless (when-csd-sans-error 
	...
	(let ((orc-end (if (< orc-beg sco-beg) sco-beg (point-max)))
	      (sco-end (if (> orc-beg sco-beg) orc-beg (point-max))))
	  (cscsd-make-template (cscsd-make-temp-name) t)
	  (cscsd-goto-orc)
	  (with-current-buffer raw-text
	    (copy-region-as-kill orc-beg orc-end))
	  (insert ?\n)
	  (yank)
	  (cscsd-goto-sco)
	  (with-current-buffer raw-text
	    (copy-region-as-kill sco-beg sco-end))
	  (insert ?\n)
	  (yank)
	  (goto-char (point-max))
	  (dolist (tag '("<CsoundSynthesizer>" "</CsoundSynthesizer>"
			 "<CsInstruments>" "</CsInstruments>" 
			  "<CsScore[^>]*>" "</CsScore>"))
	    (save-excursion
	      (when (and (re-search-backward tag nil t)
...
The function returns t if no error was found"
  (save-excursion
    ...
    (unless (and (save-excursion
                   (goto-char (point-min))
                   (and (search-forward "<CsoundSynthesizer>" nil t)
                        (search-forward "</CsoundSynthesizer>" nil t)))
		 (save-excursion
                   (goto-char (point-min))
...
(defun cscsd-buffer-is-a-csd-p (&optional buffer)
  ...
  (or (save-match-data 
	(string-match "\\.csd\\(<[0-9]+>\\)?$" (buffer-name buffer)))
      (and (boundp 'mmm-mode) mmm-mode) ; hack
      (save-current-buffer
        (set-buffer (or buffer (current-buffer)))
	(save-excursion
	  (goto-char (point-min))
...
(defun cscsd-buffer-list ()
  (remove-if-not 'cscsd-buffer-is-a-csd-p (buffer-list)))

(defun cscsd-buffer-is-a-sco-p (&optional buffer)
  "Test weither the current buffer is a score"
  (or (string-match "\\.sco\\(<[0-9]+>\\)?$" (buffer-name buffer))  
      (save-excursion
...
(defun cscsd-buffer-is-an-orc-p (&optional buffer)
  ...
  (or (string-match "\\.orc\\(<[0-9]+>\\)?$" (buffer-name buffer))
      (save-excursion
	(setq buffer (or buffer (current-buffer)))
	(eq major-mode 'csound-orc-mode))))

(defun cscsd-score-interpreter ()
  (save-excursion
    (goto-char (point-min))
    (when (and (re-search-forward "<CsScore\\([^>]*\\)>" nil t)
	       (match-string 1))
      (re-search-backward "bin=\"\\(.*\\)\"" nil t)
      (match-string-no-properties 1))))

(defun cscsd-wrap (orc sco &optional name score-binary)  
  "Create a csd from an orc plus a sco"
  (when (cscsd-make-template 
	 (setq name (if name (concat name ".csd") "temp.csd"))
	 nil
	 (or score-binary
	     (when (get-buffer sco)
	       (save-excursion
...
(defun cscsd-insert-template (&optional void score-binary)
