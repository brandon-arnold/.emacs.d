...
;  (load "calc-stef")
  ...
  (easy-menu-define scomx-menu csound-sco-mode-map
    ...
    `("Matrix"
      ...
      "--"
      ["Erase a selection"          scomx-delete-selection (scomx-matrices-p)]
      "--"
...
and OUTPUT-FILE a temporary file where a replacement for SID is read"
  (let ((input-file (cscsd-make-temp-file "csoundmx"))
	(output-file (cscsd-make-temp-file "csoundmx")))
    (unwind-protect
...
(defun scomx-delete-selection (sid)
  "empty the score selection SID of its elements, remove SID"
  (interactive "Mid: ")
  (save-excursion
    (let* ((beg-col (scomx-beg-column sid))
      ...
      (goto-char beg)
      (scomx-beginning-of-i-statement)
      (while (< (point-at-eol) (scomx-selection-end sid))
	(if (scomx-i-statement-p)
	    (progn
...
with their values as computed by calc"
  ...
  (save-excursion
    ...
    (when (or (re-search-backward ";.*" (point-at-bol) t)
	      (re-search-backward "/\\*.*" (point-at-bol) t))
      (replace-match "")) 
    (goto-char (point-at-bol))  
    (when (re-search-forward "ftgen[tmponce]*" nil t)
