...
(defmacro with-section-template (beg end body)
  (declare (indent 0))
  `(progn
     (insert ,beg)
     (save-excursion
...
;; ===================================================================
;;            the 'defCSD and 'csound-composition macros
;; ===================================================================
...
:instr 
  ...
  HEAD is the instrument number or name, TAIL are orchestra code specifications.

:format
  insert an instrument format specification
  HEAD is the instrument number or name, TAIL is the format specification.
...
:do 
  evaluate BODY after the CSD is built. BODY forms are plain lisp code.  
...
:about or :doc
 ...
 HEAD should be a string: it will be function NAME doc-string.

:SF2
 insert a soundfont-based microtonal instrument
 HEAD is a list of arguments to be passed to `make-soundfont-instrument'.
"
  ...
  (let (options comment
	...
	cscompo-csd-finalization
	cscompo-csd-post-edition
	cscompo-local-variables
	...
	((:about :doc)
	 ...
	 (dolist (fspec body)
	   (push `(csl-insert-ftable ,@fspec)
        ...
        (:format
         (push 
	  `(cscsd-insert-itemplate ,head ',@tail) cscompo-csd-finalization))
	(:do
	 (dolist (code body)
	   (push code cscompo-csd-post-edition)))
        (:opcode 
	...
	(:SF2 
	 (setq soundfont-instruments t)
	 (add-to-list 'cscompo-local-variables '%all-soundfont-instruments%)
	 (push '(require 'csound-fluid) cscompo-orc-globals)
	 (push `(add-to-list '%all-soundfont-instruments% 
			     (make-soundfont-instrument ,@head)
			     t)
	       cscompo-orc-globals))
	(t (error "Illegal csound-composition keyword: %s" key))))
    ;; soundfont instruments
    (when soundfont-instruments
	(push '(csound-set-nchnls 2) cscompo-orc-code)
	(push '(insert "\n/*| genOffset: 100 |*/\n") cscompo-orc-code)
	(push `(loop for ins in %all-soundfont-instruments%
		     for n upfrom 1
		     do (insert-soundfont-init-code ins n))
	      cscompo-orc-globals)
	(push `(loop for ins in %all-soundfont-instruments%
		     for n upfrom 1
		     do (insert-soundfont-instrument-code ins n)) 
	      cscompo-orc-code)
	(push `(loop for ins in %all-soundfont-instruments%
		     for n upfrom 1
		     do (insert-soundfont-instrument-format ins n))
	      cscompo-csd-finalization)
	(push `(insert-soundfont-instruments-output %all-soundfont-instruments%)
	      cscompo-orc-code))
    ;; building the eventual composition, a function
...
See Info node `(csound-x)Csound Elisp' for documentation."
  ...
  ;; but it failed in Emacs 24, the error being eventually related
  ;; to byte-compiled defCSD; so now we prevent the form
  ;; (csound-composition ...) to return a byte-code vector
  ;; Ok this explanation is not clear but the bug isn't clear to me either..
  ;; To reproduce: just make sure csound-lsp.elc exists: it will not load
  `(eval '(symbol-function (defCSD ,(gensym) () ,@body))))
...
symbol fbound via `defCSD', in which case it possibly accepts PARAMETERS.
Return WAVFILE"
  (let ((wfile (or wavfile (make-temp-file (cscsd-SFDIR) nil ".wav"))))
    (with-csound-to-WAV wfile
      (apply composition parameters)
      wfile)))
...
COMPOSITION is either a lambda form returned by `csound-composition' or a
symbol fbound via `defCSD', in which case it possibly accepts PARAMETERS."
  (let ((wfile (or wavfile (cscsd-make-temp-file (cscsd-SFDIR) nil ".wav"))))
    (with-csound-to-audio-editor wfile
      (apply composition parameters)
      wfile)))
