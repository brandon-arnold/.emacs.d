...
;  (add-to-list 'mmm-save-local-variables 'buffer-invisibility-spec)  
  (add-to-list 'mmm-save-local-variables 'tab-stop-list)  
  (add-to-list 'mmm-save-local-variables 'font-lock-extend-region-functions)  
  (mmm-add-classes
   '((csound-orc
     ...
     (lua
      ...
      :back "}}"
      :face mmm-code-submode-face)))
  (dolist (class '(python lua csound-sco csound-orc keykit embedded-elisp text)) ; org))
...
(dolist (spec (list '("score" . "Csound")
		    '("csd" . "Csound")
		    '("csound" . "Csound")))
  (add-to-list 'mouse-buffer-menu-mode-groups spec t))
