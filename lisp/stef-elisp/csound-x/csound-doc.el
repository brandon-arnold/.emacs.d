...
(defun csdoc-decode-opcode (opcode)
(if (string= "Ë†" opcode)
    (message "YO"))
  (or (cadr (assoc opcode csdoc-special-opcodes)) opcode))

;TEST (csdoc-decode-opcode "&gt;") => ">"
;TEST (csdoc-decode-opcode "diskin") => "diskin"
	  ...
	  (while (re-search-forward "href=\"\\([^\"]+\\)\"" end t)
	    (let* ((html-file (match-string 1))
	      ...
	      (unless (or
		       ...
		       (equal ?# (elt opcode 0))
		       (equal ?< (elt opcode 0)))
		(setq opcode (csdoc-decode-opcode opcode))
...
(defun-opcode-command csdoc-opcode-html-template 
  "Return the documentation template related to opcode at point"
  "Opcode: "
  (let ((opcode-template ""))
    (with-temp-buffer
...
(defun-opcode-command csdoc-opcode-first-html-template 
  "Return the first line of the documentation template related to opcode
 at point"
  "Opcode: "
  (car (split-string (csdoc-opcode-html-template opcode) "\n" t)))
...
WARNING: this replaces the line at point !"
  "Insert template for opcode: "
    (beginning-of-line)
    (kill-line)
    (insert (csdoc-opcode-html-template opcode)))
    ...
    (dolist (opcode (csdoc-get-all-opcodes))
	  ...
	  (if (or (not opcode-template)
		...
		(add-to-list 'csdoc-irregular-opcodes opcode))
            ...
            (let ((ok t))
              (if (string-match "(.*)" 
				(car (split-string opcode-template "\n" t)))
                  (let ((opcode-name (csdoc-decode-opcode opcode)))
                    (puthash opcode-name
...
(defun csdoc-read-deprecated (opcode)   
  (save-excursion
    (goto-char (point-min))
    (when 
	(or (re-search-forward "Same as the .* href=\"\\(.*\\)\.html\" .* opcode\." nil t)
	    (and (let ((case-fold-search nil))
		   (search-forward "Deprecated\." nil t)) 
		 (re-search-forward "href=\"\\([[:alnum:]]*\\)\.html\"" nil t)))
...
;; hippie-expand
(require 'hippie-exp)
(fset 'csdoc-expand-opcode (make-hippie-expand-function
			   '(try-csound-opcodes-templates)))
                          
(defun try-csound-opcodes-templates (old)
  (if (not old)
      (progn
	(he-init-string (he-lisp-symbol-beg) (point))
	(setq he-expand-list
	      (mapcar 'csdoc-opcode-first-html-template
		      (all-completions he-search-string csdoc-opcodes)))))
...
(defvar csound-python-opcodes nil)
(defvar csound-lua-opcodes nil)
(defvar csound-regexp-python-opcodes nil)
(defvar csound-regexp-lua-opcodes nil)
...
(defconst csdoc-opcode-typology
  '((csound-declaration-opcodes . '("instr" "endin" "opcode" "endop"))
    ...
    (csound-python-opcodes . 
'("pyassign" "pyassigni" "pylassign" "pylassigni" "pyassignt" "pylassignt"
  "pycall" "pycall1" "pycall2" "pycall3" "pycall4" "pycall5" "pycall6" "pycall7" "pycall8"
  "pycallt" "pycall1t" "pycall2t" "pycall3t" "pycall4t" "pycall5t" "pycall6t"  "pycall7t" "pycall8t"
  "pycalli" "pycall1i" "pycall2i" "pycall3i" "pycall4i" "pycall5i" "pycall6i" "pycall7i" "pycall8i"
  "pycalln" "pycallni"
  "pylcall" "pylcall1" "pylcall2" "pylcall3" "pylcall4" "pylcall5" "pylcall6" "pylcall7" "pylcall8"
  "pylcallt" "pylcall1t" "pylcall2t" "pylcall3t" "pylcall4t" "pylcall5t" "pylcall6t" "pylcall7t" "pylcall8t"
  "pylcalli" "pylcall1i" "pylcall2i" "pylcall3i" "pylcall4i" "pylcall5i" "pylcall6i" "pylcall7i" "pylcall8i"
  "pylcalln" "pylcallni"
  "pyeval" "pyevali" "pyevalt" "pyleval" "pylevali" "pylevalt"
  "pyexec" "pyexeci" "pyexect" "pylexec" "pylexeci" "pylexect"
  "pyrun" "pyruni" "pyrunt" "pylrun" "pylruni" "pylrunt"
  "pyinit"))

    (csound-regexp-python-opcodes . (csdoc-regexp-for-words
                                   csound-python-opcodes))

    (csound-lua-opcodes . 
'("lua_iopcall" "lua_ikopcall" "lua_iaopcall" "lua_iopcall_off" "lua_ikopcall_off" "lua_iaopcall_off" "lua_exec" "lua_opdef"))

    (csound-regexp-lua-opcodes . (csdoc-regexp-for-words
                                   csound-lua-opcodes))
...
(defun csound-z (&optional function)
  ...
  (erase-buffer)
  (shell-command 
   (format "\"%s\" -z" (cscsd-csound-binary)) csdoc-opcodes-buffer-name)
